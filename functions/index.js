...
app.post("/api/messages", async (req, res) => {
  ...
  try {
    ...
    functions.logger.log("Error detecting sentiment or saving message", error.message);
    res.sendStatus(500);
  }
});
...
app.get("/api/message/:messageId", async (req, res) => {
  ...
  try {
    ...
    if (!snapshot.exists()) {
      return res
        .status(404)
        .json({ errorCode: 404, errorMessage: `message '${messageId}' not found` });
    }
    res.set("Cache-Control", "private, max-age=300");
    return res.status(200).json(snapshot.val());
  } catch (error) {
    functions.logger.log("Error getting message details", messageId, error.message);

...
describe("[ClockSale]", function () {
  ...
  before(async () => {
    const [Sale, NftFactory, _accounts] = await Promise([
  ...
  describe("Configuration initialization", () => {
    ...
    it("Owner should stop marketplace", async () => {
      expect(await marketplace.paused(), "Stopped at beggining").to.be.equal(
        false
      );
      await expect(
